 ...
 *  Release History:
 *
 *    2018-06-05           Started Working on Changing this over to REOLINK Cameras
 *
...
metadata {
    definition (name: "Reolink Camera", namespace: "belgarion", author: "Belgarion") {
        capability "Actuator"
...
def take() {
    ...
    hubGetImage("/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=123456&user=${camUser}&passowrd=${camPassword}")
}
...
def videoStart() {
    ...
    if (device.currentValue('streamType') == "MJPEG") {
        ...
        apiCommand = "//h264Preview_01_main"  //For Reolink Cameras
        if (!camRTSPPort) {
            usePort = 554
        }
        else {
